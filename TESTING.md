...
# 纯 PR（无排名数据）：
info_chs: 19.02四阶魔方单次PR Teodor Zajder🇵🇱 | ...
info_eng: 19.02 4x4 PR Single Teodor Zajder🇵🇱 | ...
```

### 纪录路径（NR/CR/WR）

> NR/CR/WR 格式只由 WCA Live `recentRecords` 确定，标题关键词不参与纪录类型判断。

```
info_chs: {成绩}{中文项目名}{类型}{格式}[选手名][国旗] | {比赛名}{国旗}
info_eng: {成绩} {英文项目} {格式} {选手名}[国旗] | {比赛名}{国旗}
```
...
`test_input.csv` 包含测试用例，格式为 `Title,Uploader,Channel ID,Video URL,info_eng,info_chs`。

`Channel ID` 列可为空（测试 fallback 路径），非空时会传 `--channel-id` 给 `gen_title.py`。

### 测试命令

```powershell
# 逐行读取 test_input.csv，运行 gen_title.py，完整输出到 test_output.txt
python run_tests.py
```

输出文件 `test_output.txt` 包含每个用例的完整运行日志（含 fallback 原因）和 PASS/FAIL 判定。

> **注意**：世界排名依赖 `rankings_cache.json`，排名更新后期望值中的 `/WRxx` 数值可能随之变化，需手动更新 `test_input.csv`。
